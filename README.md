...
- ttn_osm.py is an optional replacement of ttn.py using the TopPlusOpen map (© BKG, [dl-de/by-2-0](https://www.govdata.de/dl-de/by-2-0)) to generate a custom map.
...
# What's included:
- [ttn.py](https://github.com/wryandginger/headless-HERE-TTN/blob/main/ttn.py)  - Runs nrsc5 for up to 5 minutes while the TTN weather and traffice data is received by your SDR. Once the files are received, nrsc5 stops and final images are assembled in /app/outputs/ttn
...
- [gif_ttn.py](https://github.com/wryandginger/headless-HERE-TTN/blob/main/gif_ttn.py) - Takes the TTN files located in /app/outputs/ttn and:
   - Makes a copy of the weather image and saves it in /app/outputs/ttn/gif
   - It creates a gif from the last 15 radar images
   - Uploads the TTN traffic, weather, and gif to /config/www/ttn on your Home Assistant instance
   - Cleans up / deletes the files you uploaded from the server
...
- [gif_here.py](https://github.com/wryandginger/headless-HERE-TTN/blob/main/gif_here.py) - Takes the here files located in /app/outputs/here and:
   - Makes a copy of the weather image and saves it in /app/outputs/here/gif
   - It creates a gif from the last 15 radar images
   - Uploads the TTN traffic, weather, and gif to /config/www/here on your Home Assistant instance
   - Cleans up / deletes the files you uploaded from the server
...
2. Edit ttn.py, ttn_osm.py, and/or here.py for your desired frequency and timezone. (These are automatically configured for Seattle, WA)
   ...
   - TopPlusOpen map is provided for free under this license agreement: (© BKG, [dl-de/by-2-0](https://www.govdata.de/dl-de/by-2-0), modified)
